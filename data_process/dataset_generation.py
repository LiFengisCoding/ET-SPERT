...
word_dir = "E:\\Dataset\\6-USTC-TFC\\"
word_name = "corpus-USTC.txt"

import os
os.chmod("E:\\Dataset\\6-USTC-TFC\\corpus-USTC.txt", 0o777)


def convert_pcapng_2_pcap(pcapng_path, pcapng_file, output_path):
    pcap_file = output_path + pcapng_file.replace('pcapng', 'pcap')
    # editcap.exe是Wireshark的一个应用软件，已安装，需要更改目录到其路径即可使用
    cmd = "D:\\Wireshark\\editcap.exe -F pcap %s %s"
...
def split_cap(pcap_path, pcap_file, pcap_name, pcap_label='', dataset_level='flow'):
    ...
    if dataset_level == 'flow':
        cmd = "D:\\PyCharm\\Code\\ET-SPERT\\SplitCap.exe -r %s -s session -o " + output_path
    elif dataset_level == 'packet':
        cmd = "D:\\PyCharm\\Code\\ET-SPERT\\SplitCap.exe -r %s -s packets 1 -o " + output_path
...
def generation(pcap_path, samples, features, splitcap=False, payload_length=128, payload_packet=5,
               dataset_save_path="E:\\ex_results\\", dataset_level="flow"):
    ...
    r_file_record = []
    print("\nBegin to generate features.（开始生成特征！）")
    ...
    for key in tqdm.tqdm(session_pcap_path.keys()):
            ...
            if splitcap:
                for p, d, f in os.walk(session_pcap_path[key]):
                    for file in f:
                        ...
                        if file_size < 0.1:
                            os.remove(p + "\\" + file)
                            print("remove sample: %s for its size is less than 5 KB.（样本小于0.1KB，被移除！）" % (p + "\\" + file))
                ...
                for p, d, f in os.walk(session_pcap_path[key]):
                    for file in f:
                        ...
                        if not os.path.getsize(current_file):
                            ...
                            print("current pcap %s is 0KB and delete（删除容量为0的样本）" % current_file)
                        else:
                            ...
                            try:
                                if 'TCP' in str(current_packet.res):
                                    ...
                                    if file_size < 0.1:
                                        os.remove(p + "\\" + file)
                                        print("remove UDP sample: %s for its size is less than 0.1KB.删除小于0.1KB的UCP样本" % (
                                                p + "\\" + file))
                            except Exception as e:
                                print("error in data_generation 611: scapy read pcap and analyse error"
                                      "（数据生成出错 829行：使用scapy读取pcap文件并分析时出错！")
                                os.remove(p + "\\" + file)
                                print("remove packet sample: %s for reading error."
                                      "（删除读取出错的样本！）" % (p + "\\" + file))
            if label_id[key] not in dataset:
                dataset[label_id[key]] = {
                    "samples": 0,
        ...
        if splitcap:
            continue
        target_all_files = [x[0] + "\\" + y for x in [(p, f) for p, d, f
                                                      in os.walk(session_pcap_path[key])] for y in x[1]]

        print("样本的数量：%d", len(target_all_files))
        print("取样的数量：%d", samples[label_count])
    ...
    print("数据集中所有标签下样本的数码总和：\t%d" % (all_data_number))
...
def pretrain_dataset_generation(pcap_path):
    output_split_path = "E:\\Dataset\\6-USTC-TFC\\USTC-TFC-flow分片-20个类别\\"
    pcap_output_path = "E:\\Dataset\\6-USTC-TFC\\USTC-TFC-flow分片-20个类别\\"
...
if __name__ == '__main__':
    ...
    pcap_path = "E:\\Dataset\\6-USTC-TFC\\USTC-TFC\\"
