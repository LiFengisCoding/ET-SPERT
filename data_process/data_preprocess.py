...
def combine_data_json():
    # 定义读取数据、保存数据的路径
    target_path = "D:\\PyCharm\\Code\\ET-SPERT\\dataset\\"
    target_file1 = target_path + "dataset_1.json"
    target_file2 = target_path + "dataset_2.json"
    save_file = target_path + "dataset.json"

    # 使用了Python内置的json包来加载两个JSON文件（target_file1和target_file2），并将它们存储在file1_json和file2_json两个变量中
    ...
    combined_result = {'0': {'samples': len(file1_random_samples), 'payload': {}},
...
def basic_process_1(y_dict, x_dict):
    ...
    x_result = {'len_train': x_len_train_np, 'len_test': x_len_test_np, 'len_valid': x_len_valid_np,
                'time_train': x_time_train_np, 'time_test': x_time_test_np, 'time_valid': x_time_valid_np}
    return x_result
