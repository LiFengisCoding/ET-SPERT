...
dataset_dir = "E:\\Dataset\\1-Cross-Platform(iOS)\\BERT_flow\\"


pcap_path, dataset_save_path, samples, features, dataset_level = "E:\\Dataset\\1-Cross-Platform(iOS)\\Cross-Platform(iOS)-flow分片-196个类别\\splitcap\\", \
                                                                 "E:\\Dataset\\1-Cross-Platform(iOS)\\BERT_flow\\", [
                                                                     5000], [
                                                                     "payload"], "flow"
...
def dataset_extract(model):
    ...
    try:
        if os.listdir(dataset_save_path + "dataset\\"):
            ...
            y_train, y_test, y_valid = \
                np.load(dataset_save_path + "dataset\\x_payload_train.npy", allow_pickle=True), np.load(
                    dataset_save_path + "dataset\\x_payload_test.npy", allow_pickle=True), np.load(
                    dataset_save_path + "dataset\\x_payload_valid.npy", allow_pickle=True), \
    ...
    for train_index, test_index in kfold.split(x_payload, dataset_label):
        train_indices.append(train_index)
        test_indices.append(test_index)

        valid_kfold = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)
        for valid_index, _ in valid_kfold.split(x_payload[train_index], dataset_label[train_index]):
            valid_indices.append(train_index[valid_index])

    x_payload_train, y_train = x_payload[train_indices[0]], dataset_label[train_indices[0]]
    x_payload_valid, y_valid = x_payload[valid_indices[0]], dataset_label[valid_indices[0]]
    x_payload_test, y_test = x_payload[test_indices[0]], dataset_label[test_indices[0]]
...
def count_label_number(samples):
    ...
    print("Length of dataset_length:", len(dataset_length))
    print("Length of labels:", len(labels))

    new_samples = [samples[0]] * len(dataset_length)

    for index in range(len(dataset_length)):
        if dataset_length[index] < samples[0]:
            print("label %s has less sample's number(%d) than defined samples %d" % (
                labels[index], len(labels[index]), samples[0]))
...
if __name__ == '__main__':
    ...
    for index in range(len(samples)):
        print("标签序号：(%d)———样本数量:(%d)" % (index, samples[index]))

    train_model = ["pre-train"]
    ml_experiment = 0

    dataset_extract2(train_model)
